    ...
    >
      ...
      <div className="relative aspect-[16/11] rounded-2xl overflow-hidden bg-slate-950 mb-3.5">
        ...
        <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between pointer-events-none">
          ...
          <span
            className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wide shadow-sm backdrop-blur-md border ${
              template.isFree
                ? 'bg-emerald-500/90 text-white border-emerald-400/50'
                : 'bg-amber-400 text-stone-950 border-amber-300 font-extrabold'
            }`}
          >
            {template.isFree ? 'Miễn Phí' : `${template.price.toLocaleString('vi-VN')} đ`}
          </span>
        </div>
