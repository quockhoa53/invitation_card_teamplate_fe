  ...
  return (
    <div
      ...
      className={`group relative rounded-2xl border p-3 flex flex-col justify-between transition-all duration-200 cursor-pointer ${
        isDark
          ? 'bg-[#0f1522] border-slate-800 hover:border-orange-500/60 hover:shadow-xl hover:shadow-black/40'
          : 'bg-white border-slate-200 hover:border-orange-500/50 hover:shadow-lg hover:shadow-slate-200/50'
      } hover:-translate-y-1`}
    >
      ...
      <div className="relative aspect-[16/10] rounded-xl overflow-hidden bg-slate-900 mb-3">
        ...
        <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between pointer-events-none">
          ...
          {template.isFree ? (
            <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold tracking-wide backdrop-blur-md bg-orange-500 text-white shadow-sm">
              Miễn Phí
            </span>
          ) : isOwned ? (
            <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold tracking-wide backdrop-blur-md bg-slate-800 text-white border border-slate-700 flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3 text-orange-400" /> Đã Có
            </span>
          ) : (
            <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold tracking-wide backdrop-blur-md bg-orange-500 text-white shadow-sm">
        ...
        <div className="absolute inset-0 bg-black/30 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
          <div className="w-10 h-10 rounded-full bg-orange-500 text-white flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-transform">
      ...
      <div className="space-y-2.5 flex-1 flex flex-col justify-between">
        <div>
          <h3 className={`font-editorial text-sm sm:text-base font-bold tracking-tight line-clamp-1 group-hover:text-orange-500 transition-colors ${
            isDark ? 'text-white' : 'text-slate-900'
          }`}>
            {template.title}
          </h3>
          <p className={`text-xs line-clamp-1 mt-0.5 ${isDark ? 'text-slate-300' : 'text-slate-500'}`}>
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            ...
            className={`py-2 px-2.5 rounded-xl text-xs font-semibold border flex items-center justify-center gap-1 transition flex-1 active:scale-95 ${
              isDark
                ? 'bg-slate-900 border-slate-700/80 hover:bg-slate-850 text-slate-200'
          ...
          {isOwned ? (
            <button
              ...
              className="py-2 px-2.5 rounded-xl text-xs font-semibold bg-orange-500 hover:bg-orange-600 text-white shadow-sm active:scale-95 transition flex items-center justify-center gap-1 flex-1"
            >
              <span>Tạo Thiệp</span>
